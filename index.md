...
Apache Iceberg tables, read and written from R. No DuckDB in the middle,
no CSV export — R was the last major data language without an Iceberg
client.
...
lists the surviving files before a byte is read.
...
`as_of` resolves against Iceberg’s snapshot *log*, so a snapshot a
rollback abandoned is not selected.
...
covers REST, Glue and S3.

## What works

Table spec **v1 and v2**, on `iceberg-rust` 0.10.0.
...
|  | ✅ works | not available |
|----|----|----|
| **Read** | predicate, projection and row-group pushdown · merge-on-read positional *and* equality deletes · nested `struct` / `list` / `map` · [`icebergr_scan_plan()`](https://pursuitofdatascience.github.io/icebergr/reference/icebergr_scan_plan.md) | 🦀 `limit` pushdown · 🦀 pushdown *onto* a nested field |
...
`integer`, `double`, `character`, `logical`, `Date`, `POSIXct`, `struct`
(a data frame column) and `list` (a
[`vctrs::list_of`](https://vctrs.r-lib.org/reference/list_of.html)) all
round-trip unchanged. The exceptions:
...
A CRAN binary needs no Rust toolchain. Compiling from source does:
`rustc` \>= 1.92, and a few minutes for 264 crates. The default build is
fully vendored and never touches the network; the two optional backends
...
GPL (\>= 3). Bundled Rust crates keep their own licences, listed in
`inst/NOTICE` and `LICENSE.note`.

Apache, Apache Iceberg and Iceberg are trademarks of The Apache Software
Foundation. `icebergr` is an independent community package, not
affiliated with or endorsed by the ASF, and is not one of the official
Iceberg clients. The logo is original artwork and does not use or
imitate any Apache mark.
