...
BG_TOP <- "#04131F"; BG_BOT <- "#0A2A45"
PANEL  <- "#0C2B45"; STROKE <- "#1E4E75"
INK    <- "#EAF5FC"; MUTED  <- "#8FB6D1"
CYAN   <- "#6FD4F0"; DIM    <- "#12354F"; DIMS <- "#28536F"; HOT <- "#12405C"
...
svg <- character(0); add <- function(...) svg <<- c(svg, sprintf(...))
...
chip <- function(x, y, w, h, fill, stroke, dash = FALSE)
  add('<rect x="%.0f" y="%d" width="%.0f" height="%d" rx="10" fill="%s" stroke="%s" stroke-width="2"%s/>',
...
add('<text class="m" x="%d" y="%d" fill="%s" font-size="23">%s</text>',
...
for (i in seq_along(STEPS)) {
  ...
  add('<text class="s" x="%.0f" y="%d" fill="%s" font-size="21" text-anchor="middle">%s</text>',
      ROW2$x[i] + ROW2$w[i] / 2, ROW2$y + 35, INK, STEPS[i])
  if (i < 3) add('<path d="M%.0f %d l11 8 -11 8" fill="none" stroke="%s" stroke-width="2.5" stroke-linecap="round"/>',
...
add('<path d="M%d %d l10 -14 10 14" fill="none" stroke="%s" stroke-width="2.5" stroke-linecap="round"/>',
...
roundrect <- function(x, y, w, h, r, col, border, lwd = 2, lty = 1) {
  ...
  p <- rbind(a(x + w - r, y + r,     -pi / 2, 0),        # top-right
...
png("man/figures/data-path.png", width = W * 2, height = H * 2, res = 144,
...
ga <- col2rgb(BG_TOP); gb <- col2rgb(BG_BOT)
for (i in seq_len(NB)) {
  t <- (i - .5) / NB; y0 <- (i - 1) / NB * H
  rect(0, fy(y0), W, fy(y0 + H / NB + 1), border = NA,
       col = rgb(ga[1] + (gb[1] - ga[1]) * t, ga[2] + (gb[2] - ga[2]) * t,
                 ga[3] + (gb[3] - ga[3]) * t, maxColorValue = 255))
}
...
for (i in seq_along(STEPS)) {
  ...
  if (i < 3) lines(ROW2$x[i] + ROW2$w[i] + 8 + c(0, 11, 0),
                   fy(ROW2$y + 19 + c(0, 8, 16)), col = CYAN, lwd = 2.5)
}
...
for (i in seq_along(FILES)) {
  ...
  roundrect(ROW3$x[i], ROW3$y, ROW3$w, ROW3$h, 10, if (h) HOT else DIM,
            if (h) CYAN else DIMS, lty = if (h) 1 else 2)
  for (k in 0:2)
    rect(ROW3$x[i] + 18, fy(ROW3$y + 15 + k * 14),
         ROW3$x[i] + ROW3$w - 18, fy(ROW3$y + 21 + k * 14), border = NA,
         col = adjustcolor(if (h) CYAN else MUTED,
                           if (h) c(0.95, 0.3, 0.95)[k + 1] else 0.18))
}
say(X0, ROW3$y + ROW3$h + 32,
...
cat(sprintf("data-path.svg %s bytes\ndata-path.png %s bytes\n",
