...
writeLines(c(
  ...
  paste0(sprintf(
    '<circle cx="%.0f" cy="%.0f" r="%.1f" fill="%s" opacity="%.2f"/>',
    STARS$x, STARS$y, STARS$r, INK, STARS$a
  ), collapse = ""),
  sprintf(
    '<g opacity="%.2f">%s%s</g>', KEEL_ALPHA,
    poly(KEEL_LIT, KEEL_L), poly(KEEL_SHADE, KEEL_S)
  ),
  sprintf(
    '<g clip-path="url(#keel)" stroke="%s" stroke-width="7" opacity="0.3">%s</g>',
...
png("man/figures/logo.png",
  width = W, height = round(H), bg = "transparent",
...
for (i in seq_len(nrow(STARS))) {
  symbols(STARS$x[i], fy(STARS$y[i]),
    circles = STARS$r[i], inches = FALSE,
...
fill(TIP_SHADE, ICE_SHADE)
fill(TIP_MID, ICE_MID)
fill(TIP_LIT, ICE_LIT)
